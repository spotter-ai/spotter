...
class StaticStatement(Visitable):
    """This class represents the AST node for the static effects of an action."""

    def __init__(self, formula):
        """ Construct a new StaticStmt.
        ...
        formula -- the parsed formula,
                   ...
                   compatibility etc. is checked later by the TreeVisitor
        """
        self._visitorName = "visit_statics_stmt"
        self.formula = formula  # a Formula
...
class ActionStmt(Visitable):
    ...
    def __init__(self, name, parameters, precond, default_static, statics, effect):
        """ Construct a new Action.

        Keyword arguments:
        name -- the name of the action
        parameters -- a list of variables denoting the parameters
        statics -- the fluents that remain constant when the action is performed, given as a Formula
        ...
        self.statics = statics # List of static fluents, right now a Formula
        self.precond = precond  # right now: a Formula << EffectStmt
...
def parse_statics_stmt(it):
    return _parse_precondition_or_effect(it, ":statics", StaticStatement)


def parse_unknown_stmt(it):
    return _parse_precondition_or_effect(it, ":unknown", StaticStatement)
...
def parse_action_stmt(iter):
    ...
    key = parse_keyword(iter.peek())
    if key.name == "statics":
        statics = parse_statics_stmt(iter)
    elif key.name == "unknown":
        default_static = True
        statics = parse_unknown_stmt(iter)
    else:
        statics = None

    eff = parse_effect_stmt(iter)
    return ActionStmt(name, param, pre, default_static, statics, eff)
