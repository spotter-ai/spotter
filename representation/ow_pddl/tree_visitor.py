...
from representation.ow_pddl import ow_pddl
...
class TraversePDDLDomain(PDDLVisitor):
    ...
    def __init__(self):
        ...
        self._actions = dict()
        self.domain = None
        self._objectType = ow_pddl.Type("object", None)
    ...
    def visit_type(self, node):
        ...
        if node.parent == None:
            self.set_in(node, ow_pddl.Type(node.name, "object"))
        else:
            self.set_in(node, ow_pddl.Type(node.name, node.parent))
    ...
    def visit_predicate(self, node):
        ...
        self.set_in(node, ow_pddl.Predicate(node.name, signature))
    ...
    def visit_action_stmt(self, node):
        ...
        node.statics.accept(self)
        statics = self.get_in(node.statics)
        ...
        self.set_in(node, ow_pddl.Action(node.name, signature, precond, node.default_static, statics, effect))

    def add_precond(self, precond, c):
        """Helper function for visit_precond_stmt.

        Keyword arguments:
        precond -- instance of the effect data structure
        c -- the formula representing the effect that we want to add to the
             addlist or dellist
        """
        # Needed for instance check.
        from representation.ow_pddl.parser import Variable

        nextPredicate = None
        isNegative = False
        if c.key == "not":
            # This is a negative precond, only one child allowed.
            if len(c.children) != 1:
                raise SemanticError(
                    "Error not statement with multiple " "children in precond of action"
                )
        ...
        if not nextPredicate.key in self._predicates:
            raise SemanticError(
                "Error: unknown predicate %s used in precond "
                "of action" % nextPredicate.key
            )
        if nextPredicate is None:
            raise SemanticError("Error: NoneType predicate used in precond of " "action")
        ...
        if len(nextPredicate.children) != len(predDef.signature):
            raise SemanticError(
                ...
                "predicate " + nextPredicate.key + " in precond of action"
            )
        ...
        if isNegative:
            precond.dellist.add(ow_pddl.Predicate(nextPredicate.key, signature))
        else:
            precond.addlist.add(ow_pddl.Predicate(nextPredicate.key, signature))

    def visit_precondition_stmt(self, node):
        """ Visits a PDDL precondition statement."""
        formula = node.formula
        precond = ow_pddl.Effect()
    ...
    def add_static(self, statics, c):
        """Helper function for visit_statics_stmt.

        Keyword arguments:
        statics -- a list of statics
        c -- the formula representing a predicate we want to add to the list
        """
        from representation.ow_pddl.parser import Variable
        ...
        statics.append(ow_pddl.Predicate(c.key, signature))

    def visit_statics_stmt(self, node):
        """ Visits a PDDL statics statement."""
        statics = list()
        formula = node.formula
        # For now we only allow and in the precondition.
        if formula.key == "and":
            ...
            for c in formula.children:
                ...
                if not c.key in self._predicates:
                    raise SemanticError(
                        ...
                        + c.key
                        + " used in statics of action"
                    )
                # Call helper.
                self.add_static(statics, c)
        else:
            # If not 'and' we only allow a single predicate in precondition.
            if not formula.key in self._predicates:
                raise SemanticError("Error: predicate in statics is not " "in CNF")
            # Call helper.
            self.add_static(statics, formula)
        self.set_in(node, statics)
    ...
    def add_effect(self, effect, c):
        ...
        # Add a new effect to the positive or negative effects respectively.
        if isNegative:
            effect.dellist.add(ow_pddl.Predicate(nextPredicate.key, signature))
        else:
            effect.addlist.add(ow_pddl.Predicate(nextPredicate.key, signature))

    def visit_effect_stmt(self, node):
        """ Visits a PDDL effect statement."""
        formula = node.formula
        effect = ow_pddl.Effect()
...
class TraversePDDLProblem(PDDLVisitor):
    ...
    def visit_problem_def(self, node):
        ...
        self._problemDef = ow_pddl.Problem(
    ...
    def add_goal(self, goal, c):
        ...
        if c.key == "not":
            ...
            if len(c.children) != 1:
                raise SemanticError(
                    "Error not statement with multiple " "children in goal definition"
                )
            nextPredicate = c.children[0]
            isNegative = True
        else:
        ...
        # Add a new effect to the positive or negative effects respectively.
        if isNegative:
            goal.dellist.add(ow_pddl.Predicate(nextPredicate.key, signature))
        else:
            goal.addlist.add(ow_pddl.Predicate(nextPredicate.key, signature))
