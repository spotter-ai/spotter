...
from representation.ow_pddl.ow_pddl import Predicate
from representation.task import Operator, Task
...
def ground(problem):
    ...
    # NB: vsarathy removed this because we might actually need the static predicates for other things
    statics = []
    if verbose_logging:
    ...
    goals = _get_goal_state(problem.goal)
    if verbose_logging:
        logging.debug("Goal:\n%s" % goals)

    # Collect facts from operators and include the ones from the goal
    facts = _collect_facts(operators) | goals[0] | goals[1]
    if verbose_logging:
    ...
    name = problem.name
    return Task(name, facts, init, goals, operators, statics)
...
def _collect_facts(operators):
    ...
    facts = set()
    for op in operators:
        facts |= op.add_effects | op.del_effects | op.pos_preconditions | op.neg_preconditions
    return facts
...
def _ground_action(action, type_map, statics, init):
    ...
    for param, objects in param_to_objects.items():
        for precondition_list in [action.precondition.addlist, action.precondition.dellist]:
            for pred in precondition_list:
    ...
    ops = [
        _create_operator(action, dict(assign), statics, type_map, init) for assign in assignments
    ]
...
def _create_operator(action, assignment, statics, type_map, init):
    ...
    for precondition in action.precondition.addlist:
        ...
        if predicate_name in statics:
            ...
            pos_preconditions.add(fact)
            # This precondition is not always true -> Add it
    neg_preconditions = set()
    for neg_precondition in action.precondition.dellist:
        fact = _ground_atom(neg_precondition, assignment)
        predicate_name = neg_precondition.name
        if predicate_name in statics:
            # Check if this precondition is false in the initial state
            if fact in init:
                # This precondition is never false -> Don't add operator
                return None
        else:
            # This precondition is not always true -> Add it
            neg_preconditions.add(fact)

    # First pass over statics: apply quantifiers.
    action_statics = set()
    for pred in action.statics:
        if predicate_name in statics:
            continue
        quantifier_dict = {}
        new_pred = Predicate(pred.name, list(pred.signature))
        for i, (var_name, var_types) in enumerate(pred.signature):
            if var_name == "*":
                new_var_name = "*" + str(len(quantifier_dict))
                new_pred.signature[i] = (new_var_name, var_types)
                objects = [type_map[type] for type in var_types]
                # Combine the sets into one set
                objects = set(itertools.chain(*objects))
                quantifier_dict[new_var_name] = objects
        # save a list of possible assignment tuples (param_name, object)
        domain_lists = [
            [(name, obj) for obj in objects] for name, objects in quantifier_dict.items()
        ]
        # Calculate all possible assignments
        q_assigns = itertools.product(*domain_lists)
        action_statics |= set(_ground_atom(new_pred, {**assignment, **dict(q_assign)}) for q_assign in q_assigns)

    add_effects = _ground_atoms(action.effect.addlist, assignment)
    del_effects = _ground_atoms(action.effect.dellist, assignment)

    # Add and delete effects override static effects.
    action_statics -= del_effects
    action_statics -= add_effects

    # If the same fact is added and deleted by an operator we treat it as added.
    ...
    del_effects -= frozenset(neg_preconditions)
    add_effects -= frozenset(pos_preconditions)
    args = [assignment[name] for name, types in action.signature]
    name = _get_grounded_string(action.name, args)
    return Operator(name, frozenset(pos_preconditions), frozenset(neg_preconditions), action.default_static,
                    frozenset(action_statics), add_effects, del_effects)
...
def _get_goal_state(goal):
    """Convert a goal (with positive and negative fluents) to the correct frozenset format."""
    return frozenset(_get_fact(atom) for atom in goal.addlist), frozenset(_get_fact(atom) for atom in goal.dellist)
