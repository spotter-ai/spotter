...
class SearchNode:
    ...
    def extract_regression(self):
        """
        Returns a list of actions that we relevant from the goal to the initial state
        """
        regressive_solution = []
        while self.parent is not None:
            regressive_solution.append(self.action)
            self = self.parent
        return regressive_solution
